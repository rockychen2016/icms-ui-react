...
export default function ICMSUser({ name, description, avatarUrl, size = 'md', shape = 'rounded', className, onClick }: ICMSUserProps) {
  ...
  return (
    <Container
      ...
      onClick={onClick}
      className={clsx('flex items-center gap-3', onClick ? 'cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 p-2 rounded-md' : '', className)}
    >
      <div className={clsx([sizeCls, radiusCls].join(' '), 'overflow-hidden bg-gray-100 dark:bg-gray-800 flex items-center justify-center shrink-0')}>
        {avatarUrl ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={avatarUrl} alt={name} className={clsx('w-full h-full object-cover', radiusCls)} />
        ) : (
      ...
      <div className="min-w-0 flex-shrink-0">
        <div className={clsx('text-gray-900 dark:text-gray-100 truncate', size === 'sm' ? 'text-xs' : 'text-sm', 'font-medium')}>{name}</div>
